...
class Example(QWidget):
    ...
    def initUI(self):
        ...
        self.edit_regText = QLineEdit("行为路径打点")
    ...
    def onActivated(self, text):
        global dadiantype
        if text=="关键字":
            self.edit_regText.setText("行为路径打点")
            dadiantype=0
        else:
            self.edit_regText.setText("savelogs")
            dadiantype=1
    def adb(self,n,m=''):
        t = threading.Thread(target=self.run, args=(n,m))
        t.start()
    def run(self,pi,pm):
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + ' >>> '+pi+pm)
